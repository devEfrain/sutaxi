...
function mod_main(){
    ...
    engine.adjust_title = _$( ".dragger > .title" );
    engine.chk_adjust = _$( ".check-adjust > .check-control > .check-caption" );
    engine.adjust = _$( ".dragger" ).removeClass( "hide" );
    engine.dragBox = _$( ".drag-box" );
    ...
    engine.adjust.drag({
        ...
        click: function(){
            ...
            engine.dragBox.removeClass( "ani" );
        },
        onDrag: function(){
            engine.dragBox.css( "top", this.context.getBoundingClientRect().top + 50 + "px" )
        },
        callback: function(){
            if( this.context.getBoundingClientRect().top >= topStart ){
                this.addClass( "ani" ).css( "top", "125%");
                engine.dragBox.addClass( "ani" ).css( "top", "calc(125% + 50px)")
                engine.cancelInterface.addClass( "hide" );
            } else {
                this.css( "top", "calc(100% - 473px)");
                engine.dragBox.css( "top", "calc(100% - 423px)")
            }
        }
    });
};
...
function setLangOptions(){
    ...
    engine.adjust_title.html( engine.lang.adjust.title );

    engine.chk_adjust.each( function( index ){
...
function payment( convert ){
    ...
    var percent = 0,
        ...
        if( engine.extra.card ){
            percent = parseInt( engine.pay * .10 );
        }
        ...
        if( this.attr( "value" ) !== "0" ) {
            if( this.attr( "value" ) == "4" ){
                for( var i = 0; i < engine.totalZones; i++ ){
                    ...
                    options.innerHTML = ( i + 1 ) + ": " + engine.rates.city[ i + 1 ].title;
                    origin.context.appendChild( options );
                }
...
function mod_config(){
    var chk_show_limits = _$( ".show-limits" ),
        ...
        chk_money_exchange = _$( ".money-exchange"),
   ...
   // obtenemos el status de money_exchange para configurar el check
    if( localStorage.getItem( "money_exchange") ){
        chk_money_exchange.attr( "checked", localStorage.getItem( "money_exchange") );
    }
    chk_money_exchange.event( "click", function(){
        if( localStorage.getItem( "money_exchange" ) ) {
            localStorage.removeItem( "money_exchange" );
        } else {
            localStorage.setItem( "money_exchange", engine.dollar );
        }
    } );
